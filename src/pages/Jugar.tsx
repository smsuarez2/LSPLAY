  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto' }}>
      <div style={s.gameHeader}>
        ...
        <h2 style={{ fontFamily: "'Fredoka One',cursive", fontSize: 24, color: '#3d2c6e' }}>🃏 Juego de Memoria</h2>
        <div style={{ display: 'flex', gap: 16 }}>
          <span style={s.statPill}>⏱ {fmt(seconds)}</span>
          <span style={s.statPill}>🔄 {moves} movimientos</span>
        </div>
      </div>
            ...
            <div key={card.id} onClick={() => flip(card.id)}
              ...
              {(card.flipped || card.matched) && (
                card.type === 'letter'
                  ...
                  : <span style={{ fontSize: 40 }}>{handEmojis[card.value]}</span>
              )}
  ...
  if (done) return (
    <div style={{ maxWidth: 500, margin: '0 auto' }}>
      <div style={s.winBox}>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 20, justifyContent: 'center' }}>
          <button style={s.btnPlay} onClick={() => { setCurrent(0); setScore(0); setSelected(null); setDone(false); setSeconds(0); }}>🔄 Jugar de nuevo</button>
          <button style={{ ...s.btnPlay, background: '#ede9fb', color: '#5b21b6', boxShadow: '0 4px 0 #7c3aed' }} onClick={onBack}>← Menú</button>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ maxWidth: 500, margin: '0 auto' }}>
      ...
      <div style={{ ...s.winBox, marginBottom: 20 }}>
        ...
        <span style={{ fontSize: 96, display: 'block', marginBottom: 8 }}>{handEmojis[correct]}</span>
      </div>
